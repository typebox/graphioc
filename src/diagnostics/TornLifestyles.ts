...
export class TornLifestyles extends DiagnosticRule {
  ...
  Verify() {
    ...
    for (const registration of this.registrations.values()) {
      const key =
        `${registration.implementation.name}-${registration.lifestyle}`;
  ...
  private VerifyScoped(registration: Registration<unknown>) {
    const warning =
      `'${registration.implementation.name}' registered as Scoped resolves to different instances within the same scope.`;
  ...
  private VerifySingleton(registration: Registration<unknown>) {
    const warning =
      `'${registration.implementation.name}' registered as Singleton is resolving to multiple instances.`;
